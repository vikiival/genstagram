...
export default function RankedFeed({ channel }: any) {
  ...
  async function fetchData(nextPage: any, initialLoad: boolean) {
    try {
      ...
      const feedData = await fetch("/api/openrank/feed", {
        method: "POST",
        headers: {
      ...
      console.log(feed)
      const isEmbedValid = (item: any) => item.embeds[0]?.url
      const isUrlValid = (item: any) =>
        !item.embeds[0]?.url.includes("stream.warpcast.com")
            ...
            <div className="flex flex-col space-y-8">
              {feed ? (
                feed.map((item: any, index: any) => (
                  <FeedCard
                    key={index}
                    image={item.embeds[0].url}
                    author={item.user.displayName || "anon"}
                    fid={item.fid}
                    text={item.text}
                    timestamp={item.timestamp}
                    pfp={item.user.pfp.url}
                    user={item.user}
                    score={item.cast_score}
                    calculateRank
                  />
                ))
              ) : (
